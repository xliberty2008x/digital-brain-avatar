...
class DigitalBrainOrchestrator(BaseAgent):
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        if route == "SKIP":
            ...
            # Mint once at the start of the flow.  This key survives tool
            # reconciliation and is never regenerated by an executor retry.
            ctx.session.state["journal_append_key"] = str(uuid.uuid4())
            ...
            # Read-only context only.  The server-owned append API resolves
            # the chain head immediately before the mutation.
            ...
            # Step 4: Execute the write plan once.  Blind retries can create
            # duplicate graph mutations; append reconciliation uses its stable
...
app = App(
    ...
    # Journal append reconciliation is explicit and keyed. A framework-level
    # retry can replay arbitrary post-append writes without that context, so
    # keep automatic tool retries disabled for this application.
    plugins=[],
)
