...
def compute_patch_sha256(files: Mapping[str, bytes]) -> str:
    """Digest over the closed quarantine payload (sorted names + contents)."""
    payload = {
...
def write_quarantine_bundle(
    *,
    ...
    intent_bytes = _canonical_json(dict(intent)).encode("utf-8")
    ...
    # First pass without checksums to compute patch digest over payload files.
    # Manifest must already include patch_sha256; we recompute for integrity.
    partial = {
        INTENT_FILENAME: intent_bytes,
        ARTIFACT_FILENAME: artifact_bytes,
        EVALUATION_FILENAME: evaluation_bytes,
    }
    # Embed patch digest into a canonical manifest.
    manifest_obj = dict(manifest)
    patch_sha = compute_patch_sha256(
        {
            **partial,
            MANIFEST_FILENAME: _canonical_json(
                {k: v for k, v in manifest_obj.items() if k != "patch_sha256"}
            ).encode("utf-8"),
        }
    )
    # Stable approach: patch_sha256 digests intent+artifact+evaluation+manifest_core.
    manifest_core = {
        k: v for k, v in sorted(manifest_obj.items()) if k != "patch_sha256"
    }
    core_bytes = _canonical_json(manifest_core).encode("utf-8")
    patch_sha = digest_text(
        _canonical_json(
            {
